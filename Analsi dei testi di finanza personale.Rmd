...
books %>%
  ...
  labs(y = "Parole più frequent dei sentimenti più influenti", x = NULL)
```
...
## Suddivisione in argomenti dei saggi

```{r,message=FALSE,warning=FALSE,echo=FALSE}
#
#creazione di 2 argomenti con LDA
book_lda <- books %>%
  count(Book, word, sort = TRUE) %>%
  cast_dtm(Book, word, n) %>% 
  LDA(k = 2, control = list(seed = 1000))
